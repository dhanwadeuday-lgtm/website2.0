...
(() => {
  ...
  // ---- real backend: every create/join call below hits the Vercel
  // serverless functions under /api/pair/*, backed by Redis, so pairing
  // actually works across two different people on two different devices
  // (the original prototype only worked across two tabs on one browser
  // via localStorage — that's been fully replaced).
  ...
  if (createForm){
    createForm.addEventListener('submit', async (e) => {
      ...
      const nameVal = $('#p1Name').value.trim();
      const emailVal = $('#p1Email').value.trim();
      if (!nameVal || !EMAIL_RE.test(emailVal)) {
        if (createError) { createError.textContent = 'please enter your name and a valid email.'; createError.hidden = false; }
        return;
      }
      ...
      try {
        const data = await apiPost('/api/pair/create', { name: nameVal, email: emailVal });
        myName = nameVal;
        myCode = data.code;
        ...
        // poll the server until person 2 joins with this code
        const poll = setInterval(async () => {
          try {
            const status = await apiGet(`/api/pair/status?code=${encodeURIComponent(myCode)}`);
            if (status.connected) {
              clearInterval(poll);
              becomeConnected(status.person2Name || 'them');
            }
          } catch(_) { /* transient network hiccup — keep polling */ }
        }, 3000);
      } catch (err) {
        if (createError) { createError.textContent = err.message; createError.hidden = false; }
      } finally {
  ...
  if (joinForm){
    joinForm.addEventListener('submit', async (e) => {
      ...
      try {
        const status = await apiGet(`/api/pair/status?code=${encodeURIComponent(code)}`);
        foundCode = code;
        $('#foundName').textContent = status.person1Name || 'your person';
        pairFound.hidden = false;
        gsap.from(pairFound, {opacity:0, y:16, duration:.6});
      } catch (err) {
        pairError.textContent = err.message || "hmm, that code doesn't match anything yet. double-check it?";
  ...
  if (joinBtn){
    joinBtn.addEventListener('click', async () => {
      ...
      const nameVal = $('#p2Name').value.trim();
      const emailVal = $('#p2Email').value.trim();
      if (!nameVal || !EMAIL_RE.test(emailVal)) {
        if (joinError) { joinError.textContent = 'please enter your name and a valid email.'; joinError.hidden = false; }
        return;
      }

      joinBtn.disabled = true;
      try {
        const data = await apiPost('/api/pair/join', { code: foundCode, name: nameVal, email: emailVal });
        myName = nameVal;
        $('#avatarBLabel').textContent = `${nameVal} · person 2`;
        becomeConnected(data.person1Name || 'them');
      } catch (err) {
        if (joinError) { joinError.textContent = err.message; joinError.hidden = false; }
      } finally {
