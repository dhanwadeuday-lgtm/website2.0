 ...
 */
async function createPair(name, email) {
  for (let attempt = 0; attempt < 6; attempt++) {
    const code = randomCode();
    const existing = await redis.hget(PAIRS_KEY, code);
...
async function getPair(code) {
  const raw = await redis.hget(PAIRS_KEY, code);
  if (!raw) return null;
 ...
 */
async function joinPair(code, name, email) {
 ...
 */
async function checkRateLimit(ip, max = 20, windowSeconds = 15 * 60) {
...
module.exports = { createPair, getPair, joinPair, checkRateLimit };
